...
void Geometry::init(const char* filename, float scaleSize) {
    ...
    for (int i = 0; i < size; i++) {
        ...
        //std::cout << rawVertexInputs[vertexIndices[i]].x << " ";
        vertices.push_back(rawVertexInputs[vertexIndices[i]]);
        center += vertices[vertices.size() - 1];
        vcount++;
  
//vertices.push_back(rawNormalInputs[normalIndices[i]]);
        //if(i % 3 == 0) {
            //std::cout << "f: v/" << rawTextureInputs[textureIndices[i]].x << "," << rawTextureInputs[textureIndices[i]].y << "/vn v/" << rawTextureInputs[textureIndices[i+1]].x << "," << rawTextureInputs[textureIndices[i+1]].y << "/vn v/" << rawTextureInputs[textureIndices[i+2]].x << "," << rawTextureInputs[textureIndices[i+2]].y << std::endl;
        //}
        vertices.push_back(rawTextureInputs[textureIndices[i]]);
        //normals.push_back(rawNormalInputs[normalIndices[i]]);
        //textures.push_back(rawTextureInputs[textureIndices[i]]);
        indices.push_back(i);
    }
    ...
    // 2 , GL_False, 8 * sizeof(GLfloat) [everything must do that], 24
